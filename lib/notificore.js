...
var library = function(){
	...
	/**
	 * just wrap public API of Notificore into several methods
	 * @param String key – secret key given you in order to access notificore server
	 * @return Object
	 */
	var notificore = ( key ) => {
		
		/** @var String baseUrl – Notificore server address */
		var baseUrl = 'https://app.notificore.com';
			...
			return new Promise(
				( resolve, reject ) => {
					...
					request(
						...
						( error, response, body ) => {
							if( error ){
								reject( `Notificore server rejected request on HTTP level: ${e}` );
								return;
							}
							try{
								var body = JSON.parse( body ); 
							} catch( e ){
								reject( `Unexpected response from Notificore server: ${r}` );
								return;
							}
							resolve( body );
						}
